...
df_mlr_summary_req$"ok_req_general" <- sapply(1:nrow(df_mlr_summary_req), function(x){
  
  vector_elegidos <- c(df_mlr_summary_req$"ok_req_normality_res"[x],
                       df_mlr_summary_req$"ok_req_cor_x"[x])
  
  check_elegidos <- sum(vector_elegidos == "Yes") == 2
  
  mi_respuesta <- ifelse(check_elegidos, 
                         yes = df_mlr_summary_req$"ok_req_homogeneity_res"[x],
                         no = "No")
...
if(nrow(df_mlr_only_req_ok_step01) > 0) rownames(df_mlr_only_req_ok_step01) <- 1:nrow(df_mlr_only_req_ok_step01)
...
df_mlr_only_req_ok_step01 <- data.frame()
if(nrow(df_mlr_only_req_ok_step01) > 0){
