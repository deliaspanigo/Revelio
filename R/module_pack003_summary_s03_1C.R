    ...
    function(input, output, session) {
      ...
      output$color_picker <- renderUI({
        ...
        generar_color_aleatorio <- function() {
          # Definir los caracteres posibles (0-9 y A-F)
          caracteres <- c(0:9, LETTERS[1:6])

          # Aleatorizar 6 veces entre los caracteres posibles
        ...
        div(
          colourpicker::colourInput(inputId = ns("sui_color"),
                                    label = "Color", value = color_aleatorio)
        )
      ...
      observeEvent(input$"sui_var_name", {
      ...
      control_01 <- reactive({
        ...
        #req(vector_all_colnames_database(), input$"sui_var_name", input$selected_factor_name)

        #req(input$"sui_var_name", input$selected_factor_name)

        validate(
          need(!is.null(input$"sui_var_name"),   ''),
          errorClass = "ERROR"
        )
